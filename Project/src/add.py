...
res = [*newsfeed_lenta.entries, *
       newsfeed_tass.entries, *newsfeed_vedomosti.entries]
...
df_add = pd.DataFrame({'category': categories, 'date': dates,
...
df_union.drop_duplicates(
    subset=['date', 'source'], inplace=True, ignore_index=True, keep='last')
