...
# загрузка файла
df = pd.read_csv('../data/transform_data.csv', sep=',',
...
task4 = df.groupby(['source', 'key_category']).agg(
    {'name_category': 'count'}).sort_values(by=['source', 'key_category']).reset_index()
task4.columns = ['source', 'key_category', 'count']

# Общее количество новостей из всех источников по данной категории за последние сутки
day_last = df.day.max()
task5 = df.query('day == @day_last').groupby('key_category').agg(
    {'name_category': 'count'}).sort_values(by='name_category', ascending=False).reset_index()

# Количество новостей данной категории для каждого из источников за последние сутки
task6 = df.query('day == @day_last').groupby(['source', 'key_category']).agg(
    {'name_category': 'count'}).sort_values(by=['source', 'key_category']).reset_index()
task6.columns = ['source', 'key_category', 'count_day']

# Среднее количество публикаций по данной категории в сутки
day_key_category = df.groupby(['day', 'key_category']).agg(
    {'name_category': 'count'}).reset_index()
day_key_category.columns = ['day', 'category', 'count']
task7 = day_key_category.groupby('category').agg(
...
task = task8.groupby(['category']).agg(
    {'count': 'max', 'day': 'first'}).reset_index()
task.columns = [['key_category', 'count', 'max']]
task8 = task[['key_category', 'max']]

# Количество публикаций новостей данной категории по дням недели
task9 = df.groupby(['day_of_week', 'key_category']).agg(
    {'name_category': 'count'}).reset_index()
